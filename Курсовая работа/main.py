...
class Broken_equipment(QDialog):
    ...
    def open_file(self):
        try:
            self.conn = sqlite3.connect('EquipmentRecorder.db')
            cur = self.conn.cursor()
            cur.execute("""UPDATE broken_equipment set works_guarantee = "нет"  FROM Equipment_more_detailed where DATETIME(start_of_operation, '+12 month') < CURRENT_TIMESTAMP AND Equipment_more_detailed.id= broken_equipment.id_equipment ;""")
            self.conn.commit()
            self.conn.close()
            self.conn = sqlite3.connect('EquipmentRecorder.db')
            cur = self.conn.cursor()
            cur.execute("""UPDATE broken_equipment set works_guarantee = "есть"  FROM Equipment_more_detailed where DATETIME(start_of_operation, '+12 month') > CURRENT_TIMESTAMP AND Equipment_more_detailed.id= broken_equipment.id_equipment ;""")
            self.conn.commit()
            self.conn.close()
            self.conn = sqlite3.connect('EquipmentRecorder.db')
            cur = self.conn.cursor()
...
class Equipment_prevention(QDialog):
    ...
    def open_file(self):
        try:
            ...
            self.conn = sqlite3.connect('EquipmentRecorder.db')
            cur = self.conn.cursor()
            cur.execute("INSERT INTO equipment_prevention( type_equipment ,id_equipment,start_of_operation,service_life) SELECT type_equipment, id, start_of_operation, service_life from Equipment_more_detailed where DATETIME(start_of_operation, '+12 month') < CURRENT_TIMESTAMP;")
            self.conn.commit()
            self.conn.close()
            self.conn = sqlite3.connect('EquipmentRecorder.db')
            cur = self.conn.cursor()
...
class Equipmentprevention_will_soon(QDialog):
    ...
    def open_file(self):
        try:
            ...
            self.conn = sqlite3.connect('EquipmentRecorder.db')
            cur = self.conn.cursor()
            cur.execute("INSERT INTO equipmentprevention_will_soon(type_equipment, id_equipment, start_of_operation, service_life, Days_end_life) SELECT type_equipment, id, start_of_operation, service_life, ROUND(julianday(DATETIME(start_of_operation, '+12 month')) - julianday('now')) FROM Equipment_more_detailed WHERE DATETIME(start_of_operation, '+12 month') > CURRENT_TIMESTAMP;")
            self.conn.commit()
            self.conn.close()
            self.conn = sqlite3.connect('EquipmentRecorder.db')
            cur = self.conn.cursor()
            cur.execute("""SELECT * FROM equipmentprevention_will_soon;""")
            col_name = [description[0] for description in cur.description]
            data_rows = cur.fetchall()
            self.conn.close()
            self.tableWidget.setRowCount(0)
